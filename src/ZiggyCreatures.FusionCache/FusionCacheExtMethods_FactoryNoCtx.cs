...
namespace ZiggyCreatures.Caching.Fusion
{
	public static partial class FusionCacheExtMethods
	{
		...
		{
			return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), failSafeDefaultValue, cache.CreateEntryOptions(setupAction), token);
		}
		...
		{
			return cache.GetOrSet<TValue>(key, (_, ct) => factory(ct), failSafeDefaultValue, cache.CreateEntryOptions(setupAction), token);
		}
		...
		{
			return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), default, cache.CreateEntryOptions(setupAction), token);
		}
		...
		{
			return cache.GetOrSet<TValue>(key, (_, ct) => factory(ct), default, cache.CreateEntryOptions(setupAction), token);
		}
