...
public static partial class FusionCacheExtMethods
{
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), failSafeDefaultValue, cache.GetOrCreateDefaultEntryOptions(key, true).SetDuration(duration).SetIsSafeForAdaptiveCaching(), FusionCacheInternalUtils.NoTags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), failSafeDefaultValue, cache.GetOrCreateDefaultEntryOptions(key, true).SetDuration(duration).SetIsSafeForAdaptiveCaching(), tags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), failSafeDefaultValue, cache.GetOrCreateDefaultEntryOptions(key, true).Setup(setupAction).SetIsSafeForAdaptiveCaching(), FusionCacheInternalUtils.NoTags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), failSafeDefaultValue, cache.GetOrCreateDefaultEntryOptions(key, true).Setup(setupAction).SetIsSafeForAdaptiveCaching(), tags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), default, cache.GetOrCreateDefaultEntryOptions(key, true).SetDuration(duration).SetIsSafeForAdaptiveCaching(), FusionCacheInternalUtils.NoTags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), default, cache.GetOrCreateDefaultEntryOptions(key, true).SetDuration(duration).SetIsSafeForAdaptiveCaching(), tags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), default, cache.GetOrCreateDefaultEntryOptions(key, true).Setup(setupAction).SetIsSafeForAdaptiveCaching(), FusionCacheInternalUtils.NoTags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, (_, ct) => factory(ct), default, cache.GetOrCreateDefaultEntryOptions(key, true).Setup(setupAction).SetIsSafeForAdaptiveCaching(), tags, token);
	}
