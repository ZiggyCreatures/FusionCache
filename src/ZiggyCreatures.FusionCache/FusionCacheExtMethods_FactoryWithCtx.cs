...
public static partial class FusionCacheExtMethods
{
	...
	/// <param name="token">An optional <see cref="CancellationToken"/> to cancel the operation.</param>
	/// <returns>The value in the cache, either already there or generated using the provided <paramref name="factory"/> .</returns>
	public static ValueTask<TValue> GetOrSetAsync<TValue>(this IFusionCache cache, string key, Func<FusionCacheFactoryExecutionContext<TValue>, CancellationToken, Task<TValue>> factory, MaybeValue<TValue> failSafeDefaultValue, FusionCacheEntryOptions? options, CancellationToken token)
	{
		return cache.GetOrSetAsync<TValue>(key, factory, failSafeDefaultValue, options, FusionCacheInternalUtils.NoTags, token);
	}
	...
	{
		return cache.GetOrSetAsync<TValue>(key, factory, default, options, FusionCacheInternalUtils.NoTags, token);
	}
	...
	/// <param name="token">An optional <see cref="CancellationToken"/> to cancel the operation.</param>
	/// <returns>The value in the cache, either already there or generated using the provided <paramref name="factory"/> .</returns>
	public static ValueTask<TValue> GetOrSetAsync<TValue>(this IFusionCache cache, string key, Func<FusionCacheFactoryExecutionContext<TValue>, CancellationToken, Task<TValue>> factory, FusionCacheEntryOptions? options, IEnumerable<string>? tags = null, CancellationToken token = default)
	{
		return cache.GetOrSetAsync<TValue>(key, factory, default, options, tags, token);
	}
