...
namespace ZiggyCreatures.Caching.Fusion.Events
{
		...
		: FusionCacheBaseEventsHub
	{
		...
		{
			FusionCacheInternalUtils.SafeExecuteEvent(operationId, key, _cache, SerializationError, () => new FusionCacheEntryEventArgs(key ?? string.Empty), nameof(SerializationError), _logger, _options.EventHandlingErrorsLogLevel);
		}

		internal void OnDeserializationError(string? operationId, string? key)
		{
			FusionCacheInternalUtils.SafeExecuteEvent(operationId, key, _cache, DeserializationError, () => new FusionCacheEntryEventArgs(key ?? string.Empty), nameof(DeserializationError), _logger, _options.EventHandlingErrorsLogLevel);
		}
	}
}
