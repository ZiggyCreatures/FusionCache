	...
	: FusionCacheAbstractEventsHub
{
	...
	{
		// METRIC
		Metrics.CounterBackplaneCircuitBreakerChange.Maybe()?.AddWithCommonTags(1, _cache.CacheName, _cache.InstanceId, Tags.Tag(Tags.Names.BackplaneCircuitBreakerClosed, isClosed));
	...
	{
		// METRIC
		Metrics.CounterBackplanePublish.Maybe()?.AddWithCommonTags(1, _cache.CacheName, _cache.InstanceId, Tags.Tag(Tags.Names.BackplaneMessageAction, message.Action.ToString()));
	...
	{
		// METRIC
		Metrics.CounterBackplaneReceive.Maybe()?.AddWithCommonTags(1, _cache.CacheName, _cache.InstanceId, Tags.Tag(Tags.Names.BackplaneMessageAction, message.Action.ToString()));
