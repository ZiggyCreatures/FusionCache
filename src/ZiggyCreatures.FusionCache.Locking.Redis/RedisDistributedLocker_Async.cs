...
public partial class RedisDistributedLocker
{
	...
	public async ValueTask<object?> AcquireLockAsync(string cacheName, string cacheInstanceId, string operationId, string key, TimeSpan timeout, ILogger? logger, CancellationToken token)
	{
		...
		return await _provider.AcquireLockAsync(GetLockKey(key), timeout, token).ConfigureAwait(false);
	}
	...
	public async ValueTask ReleaseLockAsync(string cacheName, string cacheInstanceId, string operationId, string key, object? lockObj, ILogger? logger, CancellationToken token)
	{
		...
		catch (Exception exc)
		{
			if (logger?.IsEnabled(LogLevel.Warning) ?? false)
				logger.Log(LogLevel.Warning, exc, "FUSION [N={CacheName} I={CacheInstanceId}] (O={CacheOperationId} K={CacheKey}): an error occurred while trying to release a distributed lock", cacheName, cacheInstanceId, operationId, key);
		}
	}
}
