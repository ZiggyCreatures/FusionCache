...
builder.Services.AddFusionCache().WithAzureServiceBusBackplane(opt =>
{
	...
	opt.SubscriptionName = "webApp1-sub";
	opt.IsAdmin = false;
});
...
class SharedDataService
{
    ...
    public async Task<string> GetDataAsync()
    {
        return await _cache.GetOrSetAsync(
            ...
            async ct =>
            {
                _logger.LogInformation("[WebApplication1] Cache miss for {CacheKey}, generating data", CacheKey);
                return $"Data from WebApplication1 at {DateTime.UtcNow:O}";
            },
    ...
    {
        _logger.LogInformation("[WebApplication1] Setting cache value: {Value}", value);
