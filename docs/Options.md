...
| `ExpirationTokens` | `IList<IChangeToken>` | `none` | This is used to set the `MemoryCacheEntryOptions.ExpirationTokens` property when saving an entry in the underlying memory cache. When Canceling the CancellationTokenSource which was used to generate the CancellationChangeToken attached to one or more cache entries, then all cache entries linked to that CancellationChangeToken with are forcibly removed. As per <a href="https://learn.microsoft.com/en-us/aspnet/core/performance/caching/memory?view=aspnetcore-6.0#cache-dependencies">Cache dependencies</a>.  Please note that setting this option on the DefaultEntryOptions may cause unintended behaviour as it the DefaultEntryOptions.ExpirationTokens would need to be refreshed after each cancellation. |
